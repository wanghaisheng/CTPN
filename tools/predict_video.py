...
def process_image(im):
    ...
    for box in text_lines:
        ...
        if present_prob > 0.5:
            cv2.putText(im, predicted_label, (left, max(0, int(top - 5))), cv2.FONT_HERSHEY_PLAIN, 2.0, (0, 255, 0), 2)
...
OUTPUT_DIR = "/home/ubuntu/testset/video"
yellow_output = '%s/output_%s.mp4' % (OUTPUT_DIR, sys.argv[1])
clip2 = VideoFileClip("%s/%s" % (OUTPUT_DIR, sys.argv[1]))
